...
from backend.services.orchestrator.prompts import build_discover_system_prompt
...
async def handle_discover(ctx: TurnContext) -> AsyncIterator[OrchestratorEvent]:
    finish_reason = "stop"
    async for event in stream_llm_response(ctx, build_discover_system_prompt(ctx)):
        if isinstance(event, FinishEvent):
            finish_reason = event.reason
        else:
            yield event

    # Run question engine after LLM stream
